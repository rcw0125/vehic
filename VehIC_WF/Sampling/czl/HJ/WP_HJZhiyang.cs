...
using VehIC_WF.Sampling.czl.HJ;

namespace VehIC_WF.WorkPoint
{
    public partial class WP_HJZhiyang : UserControl, ICardMessage
    {
        public WP_HJZhiyang()
        {
            InitializeComponent();
        ...
        private void WP_Zhiyangdan_Load(object sender, EventArgs e)
        {
            ...
            LabTabs();
            hjmixs.LoadDataByWhere("main.SAMPLESTATE=@SAMPLESTATE and main.WLLX='合金'", SampleState.组批完成);
            comboBox1.Items.Clear();
            foreach (var item in hjmixs)
            {
        ...
        private void btnSaveLab_Click(object sender, EventArgs e)
        {
            ...
            if (tempLab.CheckGroupType == "立刻检验")
            {
                ...
                {
                    if (item.CheckVal == "" || item.CheckVal == null)
                    {
                        MessageBox.Show(item.CheckItemName+"没有录入值！");
                        return;
                    }
        ...
        private void 制样_Click(object sender, EventArgs e)
        {
            ...
            if (!comboBox1.Text.StartsWith("HB")&& !comboBox1.Text.StartsWith("PB"))
            {
            ...
            if (zySample.WLLX != "合金")
            {
                MessageBox.Show("物料不是合金请选择其他制样点");
                return;
            ...
            for (int i = 0; i < MatCheckGroup.Count; i++)
            {
                ...
                if (!(zySample.SampleType == SampleType.普通样 && item.CheckGroupName == "大样留存样"))
