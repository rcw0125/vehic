...
{
    public partial class WP_HJShenhe : UserControl
    {
        public WP_HJShenhe()
        {
            InitializeComponent();
        ...
        private void loadData()
        {
            ...
            pici.LoadDataByWhere("main.BillType=@BillType and main.CheckGroupType='后续检验' and main.WLLX='合金' order by main.JyCode desc", "检验完成");
        ...
        private void 查询检验单_Click(object sender, EventArgs e)
        {
            ...
            {
                pici.LoadInfo = "CheckVals";
                pici.LoadDataByWhere("main.JyTime>=@Ksdate and  main.JyTime<=@Jsdate and main.WLLX='合金' order by main.JyCode desc", DateTime.Parse(this.dateEdit1.Text.Trim()), DateTime.Parse(this.dateEdit2.Text.Trim()));
                foreach (var item in pici)
                {
                    mcg.LoadDataByWhere("main.Sample_Lab_ID=@Sample_Lab_ID", item.Sample_Lab_ID);
                    mix.LoadDataByWhere("main.Sample_Mix_ID=@Sample_Lab_ID", mcg[0].Sample_Mix_ID);
