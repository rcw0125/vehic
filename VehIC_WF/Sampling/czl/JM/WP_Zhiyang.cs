...
{
    public partial class WP_Zhiyang : UserControl, ICardMessage
    {
        public WP_Zhiyang()
        {
            InitializeComponent();
        ...
        /// 窗体加载
        /// </summary>
        string workpointCode = "0068";
        private void WP_Zhiyangdan_Load(object sender, EventArgs e)
        {
            //LocalInfo.Current.workpoint.Code = "0068";
        ...
        void LabTabs()
        {
            ...
            zyMix.LoadDataByWhere("SampleState=@SampleState and Mix_time>=@mixtime and main.WLLX='煤'", 2, DateTime.Today);
        ...
        public void HandleCardMessage(Device.CardReader device, string cardId)
        {
            ...
            if (icCard.CardType == QC_IC_Info.CardUseType_Mix)
            {
                ...
                if (zySample.SampleType == SampleType.普通样)
                {
                    ...
                    //CheckItemView.LoadDataByWhere("MatNCid=@MatNCid", zySample.MatPK);
                ...
                {
                    if (!ckg.CheckGroupName.Contains("角质层")&&!ckg.CheckGroupName.Contains("煤岩样") && (ckg.CheckGroupType == "立刻检验" || ckg.CheckGroupType == "后续检验"))
                    {
        ...
        private void 打印详细制样编码_Click(object sender, EventArgs e)
        {
            ...
            //{
            //string ZyDanHao = "";//dataGridView2.SelectedRows[0].Cells[0].Value.ToString();
            //DbEntityTable<QC_Sample_Mix> m = new DbEntityTable<QC_Sample_Mix>();
            //m.LoadDataByWhere("ZyDanHao=@ZyDanHao", ZyDanHao);

            //DbEntityTable<QC_MixCheckGroup> ck = new DbEntityTable<QC_MixCheckGroup>();
            //ck.LoadDataByWhere("main.Sample_Mix_ID=@Sample_Mix_ID", m[0].Sample_Mix_ID);
            //for (int i = 0; i < ck.Count; i++)
            //{
            //    type.Add(QC_Sample_Mix.ShortStoreCode(ck[i].StoreCode));
            //    name.Add(ck[i].CheckGroupName);
            //}
        ...
        private void printDocument1_PrintPage(object sender, System.Drawing.Printing.PrintPageEventArgs e)
        {
            ...
            if (zySample != null && zySample.CheckGroups.Count > 0)
            {
                ...
                if (printPage >= 0 && printPage < zySample.CheckGroups.Count)
                {
                    ...
                    //barCodeControl2.Text = head;
                    //Rectangle rc = new Rectangle(9, 100, 900, 900);
                    //Bitmap bm = new Bitmap(900, 900);
                    //Rectangle rc1 = new Rectangle(0,0,900,900);
                    //barCodeControl2.DrawToBitmap(bm, rc1);
                    //Image im = bm;
                    //e.Graphics.DrawImage(im,rc);
        ...
        //private int PrintPageCapacity2=5;
        //private int PrintPages = 1;

       // private DbEntityTable<QC_Sample_Lab> jzLabs = new DbEntityTable<QC_Sample_Lab>();
        ...
        {
            //jzLabs.LoadDataByWhere("main.LabState='制样' and main.WpCode=@WpCode and main.CheckGroupName like '角质层%'", LocalInfo.Current.workpoint.Code);
            //curPrintPage2 = 1;
            //PrintPages = Convert.ToInt32(Math.Ceiling(jzLabs.Count * 1.0 / PrintPageCapacity2));
        }
        ...
        private void 查询_Click(object sender, EventArgs e)
        {
            ...
            if (!string.IsNullOrEmpty(storecode))
            {
                ...
                //labs.LoadInfo = "CheckVals";

                //labs.LoadDataByWhere("main.StoreCode=@StoreCode", QC_Sample_Mix.FullStoreCode(storecode));
                ...
                if (sellLab != null)
                {
                    ...
                    //    foreach (var item in zySample.CheckGroups)
                    //    {
                    //        if (item.Sample_Lab_ID == curSelLab.Sample_Lab_ID)
                    //        {
                    //            curSelLab.MixCheckGroup.Add(item);
                    //            item.SampleLab = curSelLab;
                    //        }
                    //    }
                    //}
        ...
        private void 复检样角质层组批_Click(object sender, EventArgs e)
        {
            if (MessageBox.Show("确实要复检样角质层组批吗?", "提示", MessageBoxButtons.OKCancel) == DialogResult.OK)
            {
                DbEntityTable<QC_MixCheckGroup> zupi = new DbEntityTable<QC_MixCheckGroup>();
               ...
               // int tbzd = 1;
                ...
                foreach (var zySample in this.zyMixSamples)
                {
                    ...
                    if (zySample.SampleType == SampleType.复检样)
                    {
                        foreach (var item in zySample.CheckGroups)
                        {
                            if ((item.CheckGroupName.StartsWith("角质层") || item.CheckGroupName.StartsWith("煤岩")) && string.IsNullOrEmpty(item.ZupiHao))
                            {
                                youJzc = true;
                            }
                        }
