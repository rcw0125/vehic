...
{
    public partial class WP_SZJianyan : UserControl, ICardMessage
    {
        ...
        private void WP_Jianyan_Load(object sender, EventArgs e)
        {
            ...
            lableiji.LoadDataByWhere("main.BillType=@BillType and main.WLLX='石子' order by JyTime desc", "开始检验");
            labTable1.LoadDataByWhere("labstate='送检' and checkgroupname like '化验样%' and  maketime>=@maketime and main.WLLX='石子'", DateTime.Today.AddDays(-1));
        ...
        public void HandleCardMessage(Device.CardReader device, string cardId)
        {
            ...
            if (tempLab.WLLX != "石子") { MessageBox.Show("此样品不是外矿"); return; }
            tempLab.JyCode = DbContext.GetSeq("WY" + DateTime.Now.Date.ToString("yyyyMMdd"), 2);
