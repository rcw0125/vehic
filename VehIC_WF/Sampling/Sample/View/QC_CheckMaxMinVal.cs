...
namespace Xg.Lab.Sample
{
    ...
    public class QC_CheckMaxMinVal : DbEntity
    {
        ...
        public static int GetHYBatchNum(string rq, string supplierCode, string matNcId)
        {
            object count = DbContext.ExecuteScalar("Select Count(*) From QC_Sample_Mix Where convert(varchar(10),Mix_Time,120)=@rq and SUPPLIERCODE=@SUPPLIERCODE and MATPK=@MATPK and (SampleType=@SampleType or SampleType=@SampleType1) ", rq, supplierCode, matNcId, (int)SampleType.机器取样,(int)SampleType.人工取样);
            return Convert.ToInt32(count);
        }
        /// <summary>
        /// 获取报检批次化验完成数量
        ...
        public static int GetHYCheckFinishBatchNum(string rq, string supplierCode, string matNcId)
        {
            object count = DbContext.ExecuteScalar("Select Count(*) From QC_Sample_Mix Where convert(varchar(10),Mix_Time,120)=@rq and SUPPLIERCODE=@SUPPLIERCODE and MATPK=@MATPK and (SampleType=@SampleType or SampleType=@SampleType1) and SampleState>=@SampleState", rq, supplierCode, matNcId, (int)SampleType.机器取样, (int)SampleType.人工取样, (int)SampleState.制样完成);
            return Convert.ToInt32(count);
        }
        ...
        public static QC_CheckMaxMinVal GetHYMaxMinVal(string rq, string supplierCode, string matNcId)
        {

            DbEntityTable<QC_CheckMaxMinVal> table = new DbEntityTable<QC_CheckMaxMinVal>();

            //string sql = "select max(cast(CheckVal as float)) as MaxVal,min(cast(CheckVal as float)) as MinVal from QC_MixSampleValue_V where SampleState>=" + (int)SampleState.化验审核完成 + " and (SampleType=" + (int)SampleType.人工取样 + " or SampleType=" + (int)SampleType.机器取样 + ") and CHECKITEMCODE='10001' and checkVal<>'' and convert(varchar(10),Mix_Time,120)=" + rq + " and SUPPLIERCODE=" + supplierCode + " and MATPK=" + matNcId;
            table.LoadDataBySql("select max(cast(CheckVal as float)) as MaxVal,min(cast(CheckVal as float)) as MinVal from QC_MixSampleValue_V where SampleState>=" + (int)SampleState.化验审核完成 + " and (SampleType=" + (int)SampleType.人工取样 + " or SampleType=" + (int)SampleType.机器取样 + ") and CHECKITEMCODE='10001' and checkVal<>'' and convert(varchar(10),Mix_Time,120)=@rq and SUPPLIERCODE=@SUPPLIERCODE and MATPK=@MATPK", rq, supplierCode, matNcId);
         //   table.LoadDataBySql(sql);
            if (table.Count > 0)
                return table[0];
            else
                return null;
